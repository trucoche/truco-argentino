...
/*
  Jugadores:  'player' | 'teammate' | 'opponent1' | 'opponent2'
  Equipos:    teamA = [player, teammate]   teamB = [opponent1, opponent2]
  Turno:      player → opponent1 → teammate → opponent2 → ...
*/

const TURN_ORDER  = ['player', 'opponent1', 'teammate', 'opponent2'];
const TEAM_A      = ['player',    'teammate'];
const TEAM_B      = ['opponent1', 'opponent2'];

export default class GameManager2v2 {
  ...
  constructor({ scene, cardManager, deckManager, config }) {
    ...
    this.cards = { player: [], teammate: [], opponent1: [], opponent2: [] };

    // Cartas jugadas en la mano actual
    this.played = { player: null, teammate: null, opponent1: null, opponent2: null };

    // Slots usados por mano (para las 3 manos de una ronda)
    this.handIndex = 0; // 0,1,2
    ...
    this.currentStage = 'envido'; // 'envido' | 'truco'

    // Managers de desafíos
    this.trucoManager  = new TrucoManager(this);
    this.envidoManager = new EnvidoManager(this);

    // Referencias a botones (se inyectan desde la escena)
    this.envidoButtons = null;
    this.trucoButtons  = null;

    // Para rastreo de limpieza
    this._allCards = [];
    this._piledCards = [];

    console.log('GameManager2v2 inicializado');
  ...
  startGame() {
    try {
      ...
      console.error('Error en startGame 2v2:', err);
...
_startTurn(who) {
  ...
  _nextTurnAfter(who) {
    ...
    // Si ya todos jugaron en esta mano → resolver
...
_playCard(card, who) {
    ...
    card.setAngle(0); // enderezar al jugar
    ...
    card.setDisplaySize(80, 120); // ← más chica al jugar
  ...
  _nextHand(handWinner) {
    ...
    // Apilar cartas jugadas — más chicas y sin transparencia
      Object.values(this.played).forEach((card, i) => {
          if (card?.scene) {
              card.setDepth(3 + i);
              this.scene.tweens.add({
                  targets:  card,
                  x:        400,      // todas al mismo centro exacto
                  y:        290,
                  displayWidth:  55,  // tamaño uniforme pequeño
                  displayHeight: 82,
                  alpha:    0.9,
                  duration: 300
              });
  ...
  _updateScoreDisplay() {
    if (this.scoreText) {
      this.scoreText.setText(
        `Vos: ${this.scores.teamA}  |  Rival: ${this.scores.teamB}`
      );
    }
  }
  ...
  showMessage(text) {
    ...
    this._lastMsg = this.scene.add.text(
      400, 290, text,
      { font: '22px Arial', fill: '#fff', backgroundColor: '#000000cc', padding: 12 }
  ...
  // Compatibilidad con EnvidoManager/TrucoManager que usan gameState
  get gameState() {
    return {
  ...
  // Compatibilidad con TrucoManager
  get opponentCards() { return [...this.cards.opponent1, ...this.cards.opponent2]; }
  get playerCards()   { return this.cards.player; }

  addScore_compat(winner, points) {
    const team = TEAM_A.includes(winner) ? 'teamA' : 'teamB';
    this.addScore(team, points, false);
  }
